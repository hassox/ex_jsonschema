defmodule Mix.Tasks.Demo do
  ...
  defp generate_markdown_formats(errors) do
    ...
    markdown_with_toc = ExJsonschema.format_errors(errors, :markdown, 
      include_toc: true, 
      heading_level: 1
    )
    File.write!("demo/errors_with_toc.md", markdown_with_toc)
    
    markdown_h3 = ExJsonschema.format_errors(errors, :markdown, 
      heading_level: 3,
      max_errors: 3
    )
  ...
  defp generate_llm_formats(errors) do
    ...
    llm_prose_no_schema = ExJsonschema.format_errors(errors, :llm, 
      include_schema_context: false
    )
    File.write!("demo/llm_prose_no_schema.txt", llm_prose_no_schema)
    
    llm_structured = ExJsonschema.format_errors(errors, :llm, 
      structured: true,
      max_errors: 10
    )
    File.write!("demo/llm_structured.txt", llm_structured)
    
    llm_structured_minimal = ExJsonschema.format_errors(errors, :llm, 
      structured: true,
      include_schema_context: false
    )
