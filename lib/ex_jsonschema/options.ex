defmodule ExJsonschema.Options do
  ...
  defp validate_draft(draft) when draft in [:auto, :draft4, :draft6, :draft7, :draft201909, :draft202012], do: :ok
  ...
  defp validate_reference_depth(depth), do: {:error, "Reference depth must be a non-negative integer, got: #{inspect(depth)}"}
  ...
  defp validate_trusted_domains(domains), do: {:error, "Trusted domains must be a list, got: #{inspect(domains)}"}
end