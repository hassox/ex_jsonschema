defmodule Mix.Tasks.Benchmark do
  ...
  defp run_benchmarks(opts) do
    ...
    invalid_instances =
      Enum.map(1..div(iterations, 10), fn i ->
        # Name too short, age might be negative
        ~s({"name": "x", "age": #{rem(i, 3) - 1}})
      end)
  ...
  defp benchmark_format(format, validator, valid_instances, invalid_instances) do
    ...
    IO.puts(
      "Average time:      #{Float.round(time_valid / length(valid_instances), 2)}μs per validation"
    )
