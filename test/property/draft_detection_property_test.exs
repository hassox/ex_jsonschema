defmodule ExJsonschema.DraftDetectionPropertyTest do
  ...
  defp valid_schema_generator do
    gen all(
          ...
          additional_props <- map_of(string(:alphanumeric), simple_value())
        ) do
  ...
  defp schema_with_draft_generator do
    gen all(
          base_schema <- valid_schema_generator(),
          draft_url <- draft_url_generator()
        ) do
  ...
  defp invalid_schema_generator do
    one_of([
      ...
      gen(
        all(base <- valid_schema_generator()) do
