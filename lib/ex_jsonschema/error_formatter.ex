defmodule ExJsonschema.ErrorFormatter do
  ...
  """
  @type format ::
          human_format() | json_format() | table_format() | markdown_format() | llm_format()
  @type format_option ::
          {:color, boolean()}
          | {:pretty, boolean()}
          | {:compact, boolean()}
  ...
  defp format_error_context(%{context: context}, color)
  ...
  defp format_error_suggestions(%{suggestions: suggestions}, color)
       when is_list(suggestions) and length(suggestions) > 0 do
  ...
  defp format_table(errors, options) do
    ...
    if Enum.empty?(display_errors) do
      "No validation errors found."
    else
  ...
  defp build_markdown_error_section(error, index, heading_level) do
    ...
    sections =
      if error.context && map_size(error.context) > 0 do
        context_lines = [
          "**Context:**",
          "",
          "```json",
    ...
    sections =
      if error.suggestions && length(error.suggestions) > 0 do
  ...
  defp build_llm_structured_error(error, index, include_schema_context) do
    ...
    schema_fields =
      if include_schema_context and error.schema_path do
    ...
    Enum.each(options, fn {key, value} ->
      case {key, format, value} do
        ...
        {:heading_level, :markdown, val} when not is_integer(val) or val < 1 or val > 6 ->
          raise ArgumentError,
                "heading_level must be an integer between 1 and 6, got: #{inspect(val)}"
        ...
        {:include_schema_context, :llm, val} when not is_boolean(val) ->
          raise ArgumentError,
                "include_schema_context option must be boolean, got: #{inspect(val)}"
