defmodule ExJsonschema.ErrorAnalyzerTest do
  ...
  describe "analyze/1" do
    ...
    test "handles empty error list" do
      ...
      assert length(analysis.recommendations) == 1  # Should have default recommendation
    end
  ...
  describe "analyze_severity/1" do
    test "classifies severity correctly for different error types" do
      errors = [
        %ValidationError{keyword: "type", instance_path: "/name"},      # critical
        %ValidationError{keyword: "required", instance_path: ""},      # critical  
        %ValidationError{keyword: "minimum", instance_path: "/age"},   # high
        %ValidationError{keyword: "format", instance_path: "/email"}   # medium
      ]
    ...
    test "considers root path errors as higher severity" do
      errors = [
        %ValidationError{keyword: "enum", instance_path: "/"},           # high (root)
        %ValidationError{keyword: "enum", instance_path: "/user/role"}   # high (same keyword)
      ]
  ...
  describe "integration with real validation errors" do
    test "analyzes errors from actual validation" do
      ...
      assert {:error, errors} = ExJsonschema.validate(validator, invalid_instance, output: :verbose)
      ...
      assert String.length(summary) > 50  # Should be reasonably detailed
    end
  end
end