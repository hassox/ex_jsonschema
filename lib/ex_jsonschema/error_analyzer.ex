defmodule ExJsonschema.ErrorAnalyzer do
  ...
  @type error_pattern :: :missing_properties | :type_conflicts | :range_violations | :format_issues
  ...
  defp categorize_error(%ValidationError{keyword: keyword}) do
    case keyword do
      k when k in ["type"] -> :type_mismatch
      k when k in ["minimum", "maximum", "minLength", "maxLength", "minItems", "maxItems", "multipleOf"] -> :constraint_violation
      k when k in ["required", "additionalProperties", "properties", "items", "uniqueItems"] -> :structural
      k when k in ["format", "pattern", "enum", "const"] -> :format
      _ -> :custom
    end
  end
  ...
  defp classify_severity(%ValidationError{keyword: keyword, instance_path: path}) do
    case keyword do
      "type" -> :critical  # Type mismatches are always critical
      "required" -> :critical  # Missing required fields are critical
      k when k in ["minimum", "maximum", "minLength", "maxLength"] -> :high
      k when k in ["format", "pattern"] -> :medium  # Format issues can often be fixed
      k when k in ["enum", "const"] -> :high  # Value must be exact
      _ -> if String.length(path) <= 1, do: :high, else: :medium  # Root errors are more severe
    end
  end
  ...
  defp detect_patterns(errors) do
    ...
    patterns = if has_missing_properties?(errors), do: [:missing_properties | patterns], else: patterns
    ...
    patterns = if has_range_violations?(errors), do: [:range_violations | patterns], else: patterns
  ...
  defp generate_recommendations(errors) do
    ...
    recommendations = if :missing_properties in patterns do
      ["Review required fields - ensure all mandatory properties are included" | recommendations]
    else
      recommendations
    end
    
    recommendations = if :type_conflicts in patterns do
      ["Check data types - multiple type mismatches detected, verify your data structure" | recommendations]
    else
      recommendations
    end
    
    recommendations = if :range_violations in patterns do
      ["Validate constraints - multiple range/length violations found, check your data bounds" | recommendations]
    else
      recommendations
    end
    
    recommendations = if :format_issues in patterns do
      ["Verify formats - format violations detected, ensure data matches expected patterns" | recommendations]
    else
      recommendations
    end
    
    # Category-based recommendations
    recommendations = if Map.get(categories, :type_mismatch, 0) > 0 do
      ["Consider data transformation - convert values to expected types" | recommendations]
    else
      recommendations
    end
    
    recommendations = if Map.get(categories, :structural, 0) > 0 do
    ...
    recommendations = if length(errors) > 5 do
      ["Consider validation early - validate data closer to its source to catch issues sooner" | recommendations]
    else
      recommendations
    end
    
    recommendations = if length(errors) > 10 do
      ["Implement data quality checks - high error count suggests systematic data issues" | recommendations]
    else
      recommendations
    end
  ...
  defp format_top_recommendations(recommendations) do
    ...
    recommendation_lines = Enum.map(top_3, fn {rec, idx} ->
      "#{idx}. #{rec}"
    end)
