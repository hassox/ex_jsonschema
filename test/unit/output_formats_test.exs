defmodule ExJsonschema.OutputFormatsTest do
  ...
  describe "output format: detailed (default error information)" do
    ...
    test "returns detailed errors for invalid instances" do
      ...
      assert name_error.message =~ "shorter than" # Actual error message format
  ...
  describe "output format: verbose (comprehensive error information)" do
    test "includes instance and schema values" do
      ...
      assert {:error, [error]} = ExJsonschema.validate(validator, ~s({"age": 15}), output: :verbose)
    ...
    test "includes validation annotations when available" do
      ...
      assert {:error, [error]} = ExJsonschema.validate(validator, ~s({"username": "x"}), output: :verbose)
  ...
  describe "validate/3 with output format options" do
    ...
    test "accepts :basic format", %{validator: validator} do
      ...
      assert {:error, :validation_failed} = ExJsonschema.validate(validator, ~s({"count": 0}), output: :basic)
    ...
    test "accepts :detailed format (default)", %{validator: validator} do
      assert {:error, errors} = ExJsonschema.validate(validator, ~s({"count": 0}), output: :detailed)
    ...
    test "accepts :verbose format", %{validator: validator} do
      assert {:error, errors} = ExJsonschema.validate(validator, ~s({"count": 0}), output: :verbose)
      assert is_list(errors)
      assert length(errors) == 1
      ...
      assert is_binary(error.schema_value) || is_integer(error.schema_value) # Schema value extraction placeholder
      assert is_map(error.context)
    end
  ...
  describe "property-based testing of output formats" do
    property "all output formats agree on validity" do
      check all schema <- simple_schema_generator(),
                instance <- json_instance_generator() do
        {:ok, validator} = ExJsonschema.compile(schema)
        
        basic_result = case ExJsonschema.validate(validator, instance, output: :basic) do
          :ok -> true
          {:error, :validation_failed} -> false
        end
        
        detailed_result = case ExJsonschema.validate(validator, instance, output: :detailed) do
          :ok -> true
          {:error, _errors} -> false
        end
        
        verbose_result = case ExJsonschema.validate(validator, instance, output: :verbose) do
          :ok -> true
          {:error, _errors} -> false
        end
    ...
    property "verbose format always includes more information than detailed" do
      check all schema <- simple_schema_generator(),
        ...
        case {detailed_result, verbose_result} do
          {:ok, :ok} -> 
            true # Both valid, nothing to compare
